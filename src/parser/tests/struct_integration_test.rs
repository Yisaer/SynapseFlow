                ...
                sqlparser::ast::Expr::JsonAccess {
                    ...
                    match inner_left.as_ref() {
                        sqlparser::ast::Expr::Identifier(ident) => {
                            assert_eq!(ident.value, "database")
                        }
...
fn test_struct_field_with_function_workflow() {
    ...
    match &field.expr {
        sqlparser::ast::Expr::Function(func) => {
            ...
            assert_eq!(
                json_access_count, 2,
                "Should have 2 JsonAccess expressions in function args"
            );
        }
    ...
    println!(
        "✓ Function call with struct field access parsed: CONCAT(user->first_name, ' ', user->last_name)"
    );
}
...
fn test_complex_struct_field_workflow() {
    ...
    match &select_stmt.select_fields[1].expr {
        sqlparser::ast::Expr::JsonAccess { .. } => {
            assert_eq!(
                select_stmt.select_fields[1].alias,
    ...
    match &select_stmt.select_fields[3].expr {
        sqlparser::ast::Expr::Function(_) => {
            assert_eq!(
                select_stmt.select_fields[3].alias,
                Some("status_msg".to_string())
            );
            println!(
                "  Field 4: Function call with struct access CONCAT('Status: ', data->status)"
            );
        }
...
fn test_error_handling_workflow() {
    ...
    assert!(
        result.is_ok(),
        "Valid struct field access should parse successfully"
    );
    ...
    assert!(
        result.is_ok(),
        "Valid complex expressions with struct access should parse"
    );
