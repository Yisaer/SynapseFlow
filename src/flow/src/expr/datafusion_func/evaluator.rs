    ...
    pub fn evaluate_df_function_vectorized(
        ...
        for row_idx in 0..num_rows {
            ...
            let physical_expr = self
                .session_ctx
                .create_physical_expr(df_expr, &df_schema_ref)?;
            ...
            ColumnarValue::Array(array) => {
                match array.len() {
                    ...
                    _ => {
                        ...
                        let datatype =
                            Arc::new(arrow_type_to_concrete_datatype(array.data_type())?);

                        Ok(Value::List(ListValue::new(values, datatype)))
                    }
                }
            }
            ColumnarValue::Scalar(scalar) => scalar_value_to_value(&scalar),
        }
    }
    ...
    fn array_element_to_value(
        ...
        let scalar_value = match array.data_type() {
            arrow::datatypes::DataType::Int64 => {
                let int_array =
                    arrow::array::as_primitive_array::<arrow::datatypes::Int64Type>(&array);
                ScalarValue::Int64(Some(int_array.value(index)))
            }
            arrow::datatypes::DataType::Int32 => {
                let int_array =
                    arrow::array::as_primitive_array::<arrow::datatypes::Int32Type>(&array);
                ScalarValue::Int32(Some(int_array.value(index)))
            }
            arrow::datatypes::DataType::Int16 => {
                let int_array =
                    arrow::array::as_primitive_array::<arrow::datatypes::Int16Type>(&array);
                ScalarValue::Int16(Some(int_array.value(index)))
            }
            arrow::datatypes::DataType::Int8 => {
                let int_array =
                    arrow::array::as_primitive_array::<arrow::datatypes::Int8Type>(&array);
                ScalarValue::Int8(Some(int_array.value(index)))
            }
            arrow::datatypes::DataType::UInt64 => {
                let uint_array =
                    arrow::array::as_primitive_array::<arrow::datatypes::UInt64Type>(&array);
                ScalarValue::UInt64(Some(uint_array.value(index)))
            }
            arrow::datatypes::DataType::UInt32 => {
                let uint_array =
                    arrow::array::as_primitive_array::<arrow::datatypes::UInt32Type>(&array);
                ScalarValue::UInt32(Some(uint_array.value(index)))
            }
            arrow::datatypes::DataType::UInt16 => {
                let uint_array =
                    arrow::array::as_primitive_array::<arrow::datatypes::UInt16Type>(&array);
                ScalarValue::UInt16(Some(uint_array.value(index)))
            }
            arrow::datatypes::DataType::UInt8 => {
                let uint_array =
                    arrow::array::as_primitive_array::<arrow::datatypes::UInt8Type>(&array);
                ScalarValue::UInt8(Some(uint_array.value(index)))
            }
            arrow::datatypes::DataType::Float64 => {
                let float_array =
                    arrow::array::as_primitive_array::<arrow::datatypes::Float64Type>(&array);
                ScalarValue::Float64(Some(float_array.value(index)))
            }
            arrow::datatypes::DataType::Float32 => {
                let float_array =
                    arrow::array::as_primitive_array::<arrow::datatypes::Float32Type>(&array);
            ...
            _ => {
                return Err(DataFusionError::NotImplemented(format!(
                    "Array element type {:?} conversion not implemented",
                    array.data_type()
                )))
            }
        };
...
mod tests {
    ...
    fn test_concat_function_vectorized() {
        ...
        let results = evaluator
            .evaluate_df_function_vectorized("concat", &args, &collection)
            .unwrap();
    ...
    fn test_upper_function_vectorized() {
        ...
        let results = evaluator
            .evaluate_df_function_vectorized("upper", &args, &collection)
            .unwrap();
...
fn arrow_type_to_concrete_datatype(
    ...
    match arrow_type {
        ...
        arrow::datatypes::DataType::Float32 => {
            Ok(ConcreteDatatype::Float32(::datatypes::Float32Type))
        }
        arrow::datatypes::DataType::Float64 => {
            Ok(ConcreteDatatype::Float64(::datatypes::Float64Type))
        }
        ...
        _ => Err(DataFusionError::NotImplemented(format!(
            "Arrow type {:?} conversion to ConcreteDatatype not implemented",
            arrow_type
        ))),
    }
}
